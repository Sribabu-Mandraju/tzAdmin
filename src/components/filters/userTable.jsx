...
const UserTable = () => {
  ...
  const filterData = () => {
    ...
    if (view === 'branch') {
      return data.filter(user => user.branch && user.branch.toUpperCase() === param.toUpperCase());
    } else if (view === 'year') {
      ...
      if (normalizedParam === 'RGUKT NUZ') {
        return data.filter(user => user.collegeId && /^N/i.test(user.collegeId));
      } else if (normalizedParam === 'RGUKT RKV') {
        return data.filter(user => user.collegeId && /^R/i.test(user.collegeId) && user.collegeId.length === 7);
      } else if (normalizedParam === 'RGUKT SKLM') {
        return data.filter(user => user.collegeId && /^S/i.test(user.collegeId) && user.collegeId.length === 7);
      } else if (normalizedParam === 'RGUKT ONG') {
        return data.filter(user => user.collegeId && /^RO/i.test(user.collegeId));
      } else {
      ...
      if (param === 'online-rgukt') {
        return data.filter(user => user.mode === 'online_mode' && rguktRegex.test(user.collegeId));
  ...
  const titleMap = {
    ...
    registrationMode: `${param === 'online-rgukt' ? 'Online RGUKT' : param === 'online-non-rgukt' ? 'Online Non-RGUKT' : 'Offline'} Registrations`,
  };
